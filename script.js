...
function playBackground(){if(!state.soundEnabled||!state.backgroundAudio||state.playing)return;state.backgroundAudio.muted=false;state.backgroundAudio.volume=.8;const result=state.backgroundAudio.play();if(result)result.catch(()=>{});}
...
document.addEventListener('pointerdown',()=>{if(state.screen!=='song'||!state.playing)playBackground()},{once:true});
